...
graph TD
    subgraph without["Без Docker"]
        A1["Твоя машина<br/>Python 3.12<br/>CUDA 12.4<br/>PyTorch 2.3"] -->|падает| X1["???"]
        A2["Его машина<br/>Python 3.10<br/>CUDA 11.8<br/>PyTorch 2.1"] -->|падает| X2["???"]
        A3["Сервер<br/>Python 3.11<br/>CUDA 12.1<br/>PyTorch 2.2"] -->|падает| X3["???"]
    end

    subgraph with_docker["С Docker — один образ везде"]
        B1["Твоя машина<br/>Python 3.12 | CUDA 12.4<br/>PyTorch 2.3 | Твой код"]
        B2["Его машина<br/>Python 3.12 | CUDA 12.4<br/>PyTorch 2.3 | Твой код"]
        B3["Сервер<br/>Python 3.12 | CUDA 12.4<br/>PyTorch 2.3 | Твой код"]
    end
```
...
Обучающий контейнер
  ...
  Запускается на GPU-кластерах. Без редактора, без Jupyter.

Инференс-контейнер
  Оптимизирован для сервинга. Маленький образ. Быстрый холодный старт.
  За балансировщиком нагрузки в проде.
```
...
# Выйди и зайди заново для применения группы
```
...
nvidia/cuda:12.4.1-devel-ubuntu22.04
  Полный CUDA toolkit. Компиляторы включены.
  Для: сборки пакетов, требующих nvcc (flash-attn, bitsandbytes)
  Размер: ~4 GB

nvidia/cuda:12.4.1-runtime-ubuntu22.04
  Только CUDA runtime. Без компиляторов.
  Для: запуска предсобранного кода
  Размер: ~1.5 GB

pytorch/pytorch:2.3.1-cuda12.4-cudnn9-runtime
  PyTorch предустановлен поверх CUDA.
  Для: пропуска шага установки PyTorch
  Размер: ~6 GB

python:3.12-slim
  Без CUDA. Только CPU.
  Для: инференса на CPU, лёгких инструментов
  Размер: ~150 MB
```
...
# Примонтировать общую директорию моделей
-v ~/models:/models

# Примонтировать датасеты
-v ~/datasets:/data
```
...
# Проверка использования GPU в запущенном контейнере
