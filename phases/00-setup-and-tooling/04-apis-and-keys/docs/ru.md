...
> Все AI API работают одинаково: отправляешь запрос, получаешь ответ. Детали меняются, принцип — нет.
...
- Распознавать и обрабатывать типичные ошибки API: аутентификацию и ограничения частоты

## Проблема

Начиная с Фазы 11 ты будешь вызывать LLM API (Anthropic, OpenAI, Google). В Фазах 13–16 — строить агентов, использующих эти API в циклах. Нужно знать, как работают ключи, как их безопасно хранить и как сделать первый вызов.
